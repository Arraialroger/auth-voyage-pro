...
export default function ManageWaitingList() {
  ...
  return <div className="min-h-screen bg-gradient-subtle overflow-x-hidden">
      ...
      <header className="border-b border-border/50 bg-card/80 backdrop-blur-sm">
        <div className="container mx-auto px-2 sm:px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-1 sm:space-x-4">
            {userType === 'receptionist' && <AddToWaitingListModal trigger={<Button variant="default" className="gap-2 text-xs sm:text-sm px-2 sm:px-4">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <Card className="bg-card/80 backdrop-blur-sm border-border/50">
            ...
            <CardContent>
              {isLoading ? <div className="text-center py-8">Carregando...</div> : waitingList.length === 0 ? <div className="text-center py-8 text-muted-foreground">
          ...
          <TableRow>
            ...
            <TableHead className="text-right">Ações</TableHead>
                  ...
                  <TableBody>
                    {waitingList.map(entry => <TableRow key={entry.id}>
                        <TableCell className="font-medium">
                          {entry.patients.full_name}
                        </TableCell>
                        <TableCell>
                          {entry.patients.contact_phone ? <a href={formatWhatsAppLink(entry.patients.contact_phone)} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 text-primary hover:text-primary/80 transition-colors group">
                            ...
                            {userType === 'receptionist' && <>
                                ...
                                <AlertDialog>
                                  <AlertDialogTrigger asChild>
                                    <Button size="sm" variant="outline" className="gap-1 hover:bg-destructive hover:text-destructive-foreground">
                              ...
                              <AlertDialogContent>
                                ...
                                <AlertDialogFooter>
                                  ...
                                  <AlertDialogAction onClick={() => handleRemoveFromWaitingList(entry.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                    Remover
                                  </AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
      ...
      {/* Schedule Appointment Modal */}
        {schedulingEntry && <NewAppointmentModal trigger={<div />} open={!!schedulingEntry} onOpenChange={open => !open && setSchedulingEntry(null)} onSuccess={handleAppointmentSuccess} initialValues={{
